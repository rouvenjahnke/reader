...
export function FilterBar({ articles, onRefresh, syncing }: { articles: ArticleSummary[]; onRefresh: () => void; syncing: boolean }): React.ReactElement {
  ...
  return (
    <div className="sticky top-0 z-20 border-b border-neutral-200 bg-[var(--background)]/95 px-3 py-3 backdrop-blur dark:border-neutral-800">
      <div className="mx-auto flex max-w-[720px] flex-col gap-3">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <Input className="pl-9" placeholder="Suchen" value={filters.query} onChange={(event) => setQuery(event.target.value)} />
          </div>
          <Button type="button" size="icon" variant="secondary" onClick={onRefresh} disabled={syncing} aria-label="Synchronisieren">
      ...
      {sourcesOpen ? (
        <div className="fixed inset-0 z-50 bg-black/30 px-3 py-20 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Quellenfilter">
          <button type="button" className="absolute inset-0 cursor-default" aria-label="Quellenfilter schliessen" onClick={() => setSourcesOpen(false)} />
          <div className="relative mx-auto flex max-h-[min(70vh,520px)] max-w-[560px] flex-col rounded-md border border-neutral-200 bg-white shadow-2xl dark:border-neutral-800 dark:bg-neutral-950">
            <div className="flex items-center justify-between gap-3 border-b border-neutral-200 px-4 py-3 dark:border-neutral-800">
              <div>
                ...
                <p className="text-xs text-neutral-500">{sources.length} verfuegbar</p>
              </div>
              <Button type="button" size="icon" variant="ghost" onClick={() => setSourcesOpen(false)} aria-label="Schliessen">
                <X className="h-4 w-4" />
              </Button>
            </div>
            <div className="overflow-y-auto p-2">
              {sources.length === 0 ? <p className="px-3 py-8 text-center text-sm text-neutral-500">Keine Quellen</p> : null}
              {sources.map((source) => (
